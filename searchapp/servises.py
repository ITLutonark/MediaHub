...
def users_search(query):
    """Возвращает пользователей в username или first_name или last_name
    которых содержится строка query"""
...
def articles_search(query):
    """Возвращает статьи в названии или тексте которых
    содержится строка query"""
    or_lookup = (Q(title__icontains=query) | Q(content__icontains=query))
    articles = Article.objects.filter(or_lookup).distinct()
    return articles
